...
import reports with counts, STEP references, and referenced record labels.
`design_variant_selection=("left hand",)` can select matching variant records
and prune the imported geometry tree by node, part, and source-name metadata.
This is useful for STEP files whose configuration labels line up with loaded
product names; deeper AP242 configuration/effectivity resolution remains
...
| `StepReadOptions` | `design_variant_selection` | Select one or more variant labels, STEP record ids, or referenced labels and prune imported geometry by matching node/part/source-name metadata. This enables name/reference-based variant geometry selection where STEP labels line up with imported product names; full AP242 effectivity resolution remains planned. |
