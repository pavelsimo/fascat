...
variables flow through simple numeric arithmetic expression operands, and
...
and intervals through selected `label=value` assignments. `STRING_LITERAL` and
`MATHS_STRING_VARIABLE` / `STRING_VARIABLE`
