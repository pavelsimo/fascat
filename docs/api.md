...
LOD generation skips parts that do not have tessellated meshes, records `lod_status="skipped_no_mesh"` on those parts, and adds report warnings so partial LOD chains are visible. Asset metadata records `lod_generated_parts` and `lod_skipped_no_mesh_parts`. LOD reports also record source, added-LOD, and full-chain vertex/triangle counts plus estimated mesh payload bytes, so the memory and export-size cost of adding extra LOD levels is visible. LOD chain advisories warn when a chain has more than four generated levels, when LOD1 or LOD2 are aggressively reduced for close or mid views, and when the farthest LOD is still geometry-only and should eventually use one-mesh/one-material baking. Per-level LOD metadata records whether source instances were reused, whether material merging or texture baking ran, whether culling granularity stayed per-part or changed because a tiny-part level was omitted, and which policy advisory applies to that level.
