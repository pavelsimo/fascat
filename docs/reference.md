...
- LOD per-level policy metadata reports whether instances were reused, whether materials were merged, whether textures were baked, and whether culling granularity changed, so current behavior is explicit while far-LOD material baking and merge workflows remain deferred.
