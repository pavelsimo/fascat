...
| Material baking | Implemented for factor/face atlas baking | `bake_materials` emits first-class raster atlas images for selected maps, glTF exports them as material texture slots, and USD exports them as `UsdUVTexture` shader networks | Add source texture resampling/dedupe and high-poly normal transfer |
