...
- `fascat validate output.glb --runtime-browser-preview preview.png` writes a browser/WebGL-rendered PNG for supported uncompressed glTF/GLB mesh primitives, including node transforms and material base-color factors. Texture sampling, Draco, meshopt-only payloads, sparse accessors, and full engine material/lighting parity remain outside this preview path.
