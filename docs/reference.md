...
| `--texture-fallback-format` | `auto` | PNG/JPEG fallback policy while KTX2/Basis is unavailable: `auto`, `png`, or `jpeg` |
...
- `--texture-fallback-format auto` records PNG fallback for alpha-bearing texture sets and JPEG fallback for color-only texture sets while KTX2/Basis output is unavailable. Explicit `jpeg` fallback reports a warning when referenced texture metadata indicates transparency would be discarded.
