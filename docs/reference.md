...
|------------|---------------|----------------------|-----------|
| CAD import, hierarchy, names, transforms, colors, metadata | Implemented for STEP and IGES; native BREP imports as a single source-shape part | `import` report stats, cleanup counts, STEP import decisions, pipeline import options, per-part loaded-representation reports, space normalization transforms, source texture resolved/missing counts, CAD material PBR mapping metadata, and AP242 PMI warnings when typed PMI import is unavailable | Add real design variant loading, typed PMI entity extraction, mixed BREP construction-curve cleanup, true multi-file import, and vendor-specific embedded material-library containers |
