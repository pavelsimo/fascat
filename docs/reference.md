...
| Material baking | Implemented for factor/face atlas baking and imported sidecar texture slots | `bake_materials` emits first-class raster atlas images for selected maps, glTF exports baked or imported source images as material texture slots, USD exports them as `UsdUVTexture` shader networks, and `process_textures` resizes/dedupes/fallback-converts first-class images | Add high-poly normal transfer and richer vendor material-library mapping |
