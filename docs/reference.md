...
| `fascat validate output.glb --runtime-engine unity --runtime-engine-project Harness/` | Measure optional Unity or Unreal harness load and FPS for glTF/GLB output |
...
| `--material-library` | unset | Vendor material-library JSON/MTL file or folder to apply during import; may be passed more than once |
...
- `fascat validate output.glb --runtime-engine unity --runtime-engine-project Harness/` or `--runtime-engine unreal --runtime-engine-project Harness.uproject` launches a local project-backed engine harness and records measured load time, FPS, frame count, memory bytes, engine version, mesh count, and triangle count. Use `--runtime-engine-command` or `FASCAT_UNITY` / `UNITY_EDITOR` / `FASCAT_UNREAL` / `UNREAL_EDITOR` when the engine executable is not on `PATH`; missing engines or projects are reported as unavailable.
...
| `--runtime-engine-project` | unset | Unity project folder or Unreal `.uproject` containing the Fascat runtime harness |
