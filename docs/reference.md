...
| BREP healing | Partial | `heal_brep`; records open shells, free/unstitched edges, small edges, sliver counts, same-domain face/edge reductions, and OCCT same-domain cleanup status; sliver removal warns that the backend leaves shapes unchanged | Implement arbitrary overlap/z-fighting cleanup, sliver-face removal, and deeper face/wire repair |
