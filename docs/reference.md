...
|------------|---------------|----------------------|-----------|
| CAD import, hierarchy, names, transforms, colors, metadata | Implemented for STEP and IGES; native BREP imports as a single source-shape part; explicit multi-root STEP path lists are imported with deterministic member namespaces | `import` report stats, cleanup counts, STEP import decisions, pipeline import options, per-part loaded-representation reports, space normalization transforms, source texture resolved/missing counts, CAD material PBR mapping metadata, AP242 PMI warnings when typed PMI import is unavailable, and `read_step_many` member records/warnings for explicit multi-file assemblies | Add real design variant loading, typed PMI entity extraction, mixed BREP construction-curve cleanup, external-reference STEP graph resolution, and vendor-specific embedded material-library containers |
