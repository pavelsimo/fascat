...
| **Validation** | output validators, geometry analysis, profile budget estimates, optional headless browser/WebGL glTF runtime measurement, optional browser/WebGL rendered screenshots for supported glTF/GLB primitives with quantized attribute support, meshopt fallback-buffer support, and base-color texture sampling, optional packaged or project-backed Unity/Unreal runtime harness drivers with custom-harness preview artifact reporting, deterministic software preview PNGs and LOD contact sheets | browser runtime harness uses a bounded triangle proxy workload; browser screenshots do not yet decode Draco, meshopt-only payloads without fallback data, or KTX2-only textures; packaged Unity/Unreal templates measure engine-process file load/parse rather than full renderer FPS and report preview requests as unavailable; preview renderer is orthographic software shading | bundled Unity/Unreal material/lighting renderer validation |
...
- **Runtime validation**: reports include measured pipeline/write/validate timings and local memory/load/frame/FPS estimates, plus optional headless browser/WebGL and packaged or project-backed Unity/Unreal glTF runtime harness drivers. Project-backed engine harnesses can now receive a requested preview PNG path and report render-status fields; packaged engine templates validate command wiring and engine-process glTF/GLB load/parse metrics while marking preview requests unavailable. Full bundled renderer parity is still open.
- **Visual validation**: deterministic before/after software preview PNGs, LOD switching contact sheets, baseline image-diff thresholds, and browser/WebGL rendered screenshots for supported glTF/GLB primitives exist with quantized vertex attributes, meshopt fallback-buffer support, and base-color texture sampling for supported image URI/data URI textures; Draco browser rendering, meshopt-only/no-fallback payloads, KTX2-only texture rendering, and full Unity/Unreal renderer screenshots are still open.
