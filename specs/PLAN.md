...
- [~] Design-variant import: common STEP configuration/design-variant/effectivity records are reported as metadata when `design_variants=True`; `design_variant_selection` can select labels/STEP record ids/referenced labels and prune imported geometry by matching node, part, and source-name metadata. Full AP242 variant/effectivity geometry resolution remains open. Updated 2026-06-01.
