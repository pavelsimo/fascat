...
| **Materials** | per-face colors + PBR factors preserved, first-class image resources, raster material atlas baking, source image resize/dedupe/PNG-JPEG fallback processing, JSON/MTL vendor material-library sidecar import | source texture sidecar extraction, CAD material-name PBR rule mapping, sampled AO bake | high-poly transfer, embedded/proprietary vendor material-library containers |
...
| **Validation** | output validators, geometry analysis, profile budget estimates, optional headless browser/WebGL glTF runtime measurement, optional packaged or project-backed Unity/Unreal runtime harness drivers, deterministic software preview PNGs and LOD contact sheets | browser harness uses a bounded triangle proxy workload; packaged Unity/Unreal templates measure engine-process file load/parse rather than full renderer FPS; preview renderer is orthographic software shading | full material/lighting renderer validation |
...
- [~] Source texture pipeline: CAD import now extracts referenced sidecar PNG/JPEG/KTX2 files and binds semantic slots; JSON/MTL material-library sidecars can also load texture slots. Embedded vendor-specific material-library containers are still partial. Updated 2026-05-31.
- [ ] AO bake to texture and/or vertex colors feeding decimation weights.
- [~] Material-library import + CAD-material-to-PBR mapping tables + diagnostics: XDE visual material values, common CAD material-name rules, and JSON/MTL sidecar material libraries are mapped to PBR factors and texture slots with resolved/matched/unmatched diagnostics; embedded/proprietary vendor material-library containers are still open. Updated 2026-05-31.
