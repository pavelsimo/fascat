...
- [x] **P2** Consistency pass: properties vs methods (`stats()`, `draw_call_breakdown()` vs `triangle_count`); reconcile `select()`-for-inspection vs `where=`-for-scoping; rename/fold `run_lod_generators` into `lods`; hide `copy(keep_source=…)`
