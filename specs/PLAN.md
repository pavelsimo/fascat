...
| **Validation** | output validators, geometry analysis, profile budget estimates, optional headless browser/WebGL glTF runtime measurement, optional browser/WebGL rendered screenshots for supported glTF/GLB primitives with quantized attribute support, meshopt fallback-buffer support, and base-color texture sampling, optional packaged or project-backed Unity/Unreal runtime harness drivers with Unity/glTFast preview screenshots and custom-harness preview artifact reporting, deterministic software preview PNGs and LOD contact sheets | browser runtime harness uses a bounded triangle proxy workload; browser screenshots do not yet decode Draco, meshopt-only payloads without fallback data, or KTX2-only textures; packaged Unity preview is a single camera render, packaged Unreal still measures engine-process file load/parse and reports preview requests as unavailable; preview renderer is orthographic software shading | bundled Unreal renderer screenshots, multi-frame engine FPS, and Unity/Unreal material/lighting parity thresholds |
...
- [~] Runtime profiling: local measured timings, optional headless browser/WebGL glTF load/FPS measurements, and optional packaged or project-backed Unity/Unreal harness drivers exist; the packaged Unity harness can return a glTFast camera preview PNG, but multi-frame engine FPS, packaged Unreal rendering, and material/lighting parity thresholds are still open. Updated 2026-06-01.
...
- [~] Real Unity/Unreal/browser runtime load-time, memory, and FPS harness: browser/WebGL glTF harness exists with bounded triangle proxy workload; Unity/Unreal command drivers can use packaged temporary harness templates or configured Fascat runtime harness projects, and custom harnesses can receive preview PNG paths plus return render-status fields. The packaged Unity harness now uses glTFast to render a single preview PNG; packaged Unreal rendering and bundled renderer FPS remain open. Updated 2026-06-01.
