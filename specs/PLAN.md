...
| **Validation** | output validators, geometry analysis, profile budget estimates, optional headless browser/WebGL glTF runtime measurement, optional browser/WebGL rendered screenshots for supported glTF/GLB primitives with quantized attribute support, meshopt fallback-buffer support, base-color texture sampling, and compressed-extension preflight diagnostics, optional packaged or project-backed Unity/Unreal runtime harness drivers with Unity/glTFast preview screenshots/FPS and custom-harness preview artifact reporting, deterministic software preview PNGs and LOD contact sheets | browser runtime harness uses a bounded triangle proxy workload; browser screenshots preflight Draco, meshopt-only/no-fallback, and KTX2/Basis limitations but do not yet decode those payloads; packaged Unity preview is a fixed multi-frame camera render, packaged Unreal still measures engine-process file load/parse and reports preview requests as unavailable; preview renderer is orthographic software shading | bundled Unreal renderer screenshots, Unreal multi-frame renderer FPS, and Unity/Unreal material/lighting parity thresholds |
...
- **Runtime validation**: reports include measured pipeline/write/validate timings and local memory/load/frame/FPS estimates, plus optional headless browser/WebGL and packaged or project-backed Unity/Unreal glTF runtime harness drivers. The packaged Unity harness can now load and instantiate GLB/GLTF through glTFast, render a fixed multi-frame camera loop when graphics are available, write the requested preview PNG, and report render-status/FPS fields; project-backed harnesses can return the same preview contract. Packaged Unreal remains load/parse only, and full bundled renderer parity is still open.
- **Visual validation**: deterministic before/after software preview PNGs, LOD switching contact sheets, baseline image-diff thresholds, and browser/WebGL rendered screenshots for supported glTF/GLB primitives exist with quantized vertex attributes, meshopt fallback-buffer support, base-color texture sampling for supported image URI/data URI textures, and explicit unsupported/partial reports for Draco, meshopt-only/no-fallback, and KTX2/Basis limitations; Draco browser rendering, meshopt-only/no-fallback decoding, KTX2-only texture rendering, and full Unity/Unreal renderer screenshots are still open.
...
- [~] Runtime profiling: local measured timings, optional headless browser/WebGL glTF load/FPS measurements, and optional packaged or project-backed Unity/Unreal harness drivers exist; the packaged Unity harness can return a glTFast camera preview PNG plus fixed multi-frame render FPS, but packaged Unreal rendering/FPS and material/lighting parity thresholds are still open. Updated 2026-06-01.
- [~] Visual before/after preview renders and LOD switching checks: deterministic software preview PNGs, output validation previews, LOD contact sheets, baseline image-diff thresholds, and browser/WebGL screenshots for supported glTF/GLB primitives exist; browser screenshots now handle quantized vertex attributes, meshopt exports with fallback buffer data, and supported base-color image URI/data URI textures, while preflight reports unsupported Draco and meshopt-only/no-fallback geometry plus partial KTX2/Basis texture previews. Draco decoding, meshopt-only/no-fallback decoding, KTX2-only texture rendering, and full Unity/Unreal renderer screenshots remain open. Updated 2026-06-01.
- [~] Real Unity/Unreal/browser runtime load-time, memory, and FPS harness: browser/WebGL glTF harness exists with bounded triangle proxy workload; Unity/Unreal command drivers can use packaged temporary harness templates or configured Fascat runtime harness projects, and custom harnesses can receive preview PNG paths plus return render-status fields. The packaged Unity harness now uses glTFast to render a fixed multi-frame preview PNG/FPS loop; packaged Unreal rendering and bundled Unreal renderer FPS remain open. Updated 2026-06-01.
