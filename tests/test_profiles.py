...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda: WorkflowRecipeChoice(stage="", setting="uv0", value="box"), "stage"),
        (lambda: WorkflowRecipeChoice(stage="stage", setting="", value="box"), "setting"),
        (lambda: WorkflowRecipeChoice(stage="stage", setting="uv0", value="box", status="bad"), "status"),
        (lambda: WorkflowRecipe(name="", target="web", description="web recipe"), "name"),
        (lambda: MergeVerticesOptions(tolerance=-1), "merge vertices tolerance"),
        (lambda: MergeVerticesOptions(area_epsilon=-1), "area_epsilon"),
        (lambda: DeleteDegeneratePolygonsOptions(area_epsilon=-1), "area_epsilon"),
        ...
        (lambda: SceneOptimizeOptions(instance_similarity_tolerance=-1), "instance_similarity_tolerance"),
        ...
        (lambda: UnwrapOptions(texel_density=0), "texel_density"),
        (lambda: UnwrapOptions(padding=-1), "padding"),
        (lambda: UnwrapOptions(max_stretch=-1), "max_stretch"),
        (lambda: UnwrapOptions(method="bad"), "unwrap method"),
        (lambda: UnwrapOptions(iterations=0), "unwrap iterations"),
        (lambda: UnwrapOptions(tolerance=-1), "unwrap tolerance"),
        (lambda: AtlasOptions(max_size=0), "atlas max_size"),
        ...
        (lambda: SceneOptimizeOptions(max_vertices_per_mesh=0), "max_vertices_per_mesh"),
        (lambda: SceneOptimizeOptions(split_large_meshes=True, max_vertices_per_mesh=2), "max_vertices_per_mesh"),
        (lambda: SceneOptimizeOptions(index_buffer="bad"), "index_buffer"),
        (lambda: SceneOptimizeOptions(flatten="bad"), "flatten"),
        (lambda: SceneOptimizeOptions(instance_policy="bad"), "instance_policy"),
        (lambda: StepReadOptions(material_library_color_space="bad"), "material_library_color_space"),
        (lambda: BakeMaterialOptions(maps_resolution=0), "maps_resolution"),
        (lambda: BakeMaterialOptions(bake=("bad",)), "unsupported bake maps"),
        (lambda: BakeMaterialOptions(ambient_occlusion_strategy="bad"), "ambient_occlusion_strategy"),
        ...
        (lambda: fc.DecimateOptions(ambient_occlusion_strategy="bad"), "ambient_occlusion_strategy"),
