...
def test_merge_vertices_asset_operation_reports_counts() -> None:
    ...
    assert step.after["merge_vertices_candidate_exact_duplicate_buckets"] == 1
...
def test_merge_vertices_asset_operation_warns_on_large_tolerance() -> None:
    ...
    merged = asset.merge_vertices(MergeVerticesOptions(tolerance=0.3))
    part = merged.parts["part"]
    step = merged.report.steps[-1]
...
def test_convert_reuses_fbx_writer_validation_stats(monkeypatch, tmp_path: Path) -> None:  # type: ignore[no-untyped-def]
    ...
    assert steps["write"].options["format"] == "FBX"
    assert steps["validate"].after["validated_meshes"] == 1
    assert steps["validate"].after["validated_points"] == 15
    assert steps["validate"].after["validated_triangles"] == 5
