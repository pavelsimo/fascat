...
_IMPORT_KEYS = frozenset(
    {
        ...
        "material_library_color_space",
        "delete_free_vertices",
        "delete_lines",
        "construction_curve_policy",
        "construction_curve_tube_radius",
        "source_units",
...
def _import_options(value: object, location: _TomlLocation | None = None) -> StepReadOptions | None:
    ...
    try:
        ...
        return StepReadOptions(
            ...
            design_variant_selection=cast(Any, value.get("design_variant_selection", ())),
            ...
            material_library_paths=cast(Any, value.get("material_library_paths", ())),
            material_library_color_space=cast(Any, _literal(value.get("material_library_color_space", "auto"))),
            delete_free_vertices=bool(value.get("delete_free_vertices", False)),
            delete_lines=bool(value.get("delete_lines", False)),
            construction_curve_policy=cast(Any, _literal(value.get("construction_curve_policy", "preserve_metadata"))),
            construction_curve_tube_radius=float(value.get("construction_curve_tube_radius", 0.01)),
...
def _tessellation(values: dict[str, object]) -> TessellationOptions:
    return TessellationOptions(
        ...
        detail_adaptive=bool(values.get("detail_adaptive", False)),
...
def _brep_heal_options(values: dict[str, object]) -> BrepHealOptions:
    return BrepHealOptions(
        ...
        remove_overlapping_faces=bool(values.get("remove_overlapping_faces", True)),
        overlap_area_ratio=_as_float(values.get("overlap_area_ratio", 0.995)),
        remove_sliver_faces=bool(values.get("remove_sliver_faces", False)),
