...
def _write_usd_stage(
    ...
    if not np.allclose(asset.root.transform, np.eye(4)):
        from pxr import Gf

        # Root carries the space normalization (units/up-axis); fascat
        # matrices are column-vector, Gf.Matrix4d is row-vector.
        scene.AddTransformOp().Set(Gf.Matrix4d(asset.root.transform.T.tolist()))
    if debug:
    ...
    # "auto" reaching the exporter (direct write_usd callers) means instanced,
    # the spec-conformant layout; the pipeline resolves "auto" per profile.
    flat = opts.layout == "flat"
    ...
    ctx = _SceneWriteContext(
        parts=asset.parts,
        materials=asset.materials,
        material_paths=material_paths,
        pmi_by_part=pmi_by_part,
        prototype_paths=prototype_paths,
        occurrence_counts=_part_occurrence_counts(asset.root),
        metadata_options=opts.metadata,
        report=asset.report,
        flat=flat,
    )
    _write_node(stage, asset.root, scene_path, ctx)
    if opts.metadata.pmi != "none":
...
def _set_part_custom_data(
    ...
    prim.SetCustomDataByKey("fascat:partId", part.id)
    prim.SetCustomDataByKey("fascat:originalName", part.name)
    _set_metadata_custom_data(prim, part.metadata, metadata_options)
    pmi_ids = pmi_by_part.get(part.id, [])
    if pmi_ids:
        prim.SetCustomDataByKey("fascat:pmiIds", Vt.StringArray(pmi_ids))
...
def _write_prototypes(
    ...
    for part in parts.values():
        ...
        for lod_index, mesh in enumerate(meshes):
            ...
            _set_part_custom_data(prototype.GetPrim(), part, pmi_by_part, metadata_options)
...
def _write_node(stage: Any, node: Node, parent_path: str, ctx: _SceneWriteContext) -> None:
    ...
    for child in node.children:
        ...
        if not np.allclose(child.transform, np.eye(4)):
            # fascat matrices are column-vector; Gf.Matrix4d is row-vector.
            xform.AddTransformOp().Set(Gf.Matrix4d(child.transform.T.tolist()))
        xform.GetPrim().SetCustomDataByKey("fascat:originalName", child.name)
        xform.GetPrim().SetCustomDataByKey("fascat:nodeId", child.id)
        _set_metadata_custom_data(xform.GetPrim(), child.metadata, ctx.metadata_options)
        if ctx.flat:
            part = ctx.parts.get(child.part_id) if child.part_id is not None else None
            if part is not None and part.mesh is not None:
                _set_part_custom_data(xform.GetPrim(), part, ctx.pmi_by_part, ctx.metadata_options)
                _write_mesh(
                    stage,
                    f"{path}/Mesh",
                    part,
                    ...
                    ctx.materials,
                    ctx.material_paths,
                    ctx.metadata_options,
                    ctx.report,
                )
        elif child.part_id is not None and (child.part_id, 0) in ctx.prototype_paths:
            part = ctx.parts[child.part_id]
            prim = xform.GetPrim()
            if part.lod_meshes:
                ...
                for lod_index in range(len(part.lod_meshes) + 1):
                    ...
                    with variant_set.GetVariantEditContext():
                        ...
                        references.AddInternalReference(Sdf.Path(ctx.prototype_paths[(child.part_id, lod_index)]))
                variant_set.SetVariantSelection("lod0")
            else:
                prim.GetReferences().AddInternalReference(Sdf.Path(ctx.prototype_paths[(child.part_id, 0)]))
            if ctx.occurrence_counts.get(child.part_id, 0) > 1:
                prim.SetInstanceable(True)
        _write_node(stage, child, path, ctx)
